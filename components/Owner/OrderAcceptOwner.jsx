...
const OrderAcceptOwner = () => {
  ...
  // Get unique routes from users
  const uniqueRoutes = ['All', ...Array.from(new Set(users.map(u => u.route)))];
  ...
  useEffect(() => {
    if (selectAllOrders) {
      let allOrderIds = {};
      orders.forEach(order => allOrderIds[order.id] = true);
  ...
  }, [searchQuery, users, selectedRoute]);
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      await Promise.all([
        fetchAllUsers(userAuthToken),
        fetchAllOrders(userAuthToken)
      ]);
      setSelectedOrderIds({});
  ...
  const fetchAllUsers = async (userAuthToken) => {
    try {
  ...
  const fetchAllOrders = async (userAuthToken) => {
  ...
  const handleOrderCardPress = (order) => {
    ...
    // Navigate to Reports tab first, then to OrderHistoryOwner with proper params
    navigation.navigate('Reports', {
      screen: 'OrderHistoryOwner',
      params: { 
...
export default OrderAcceptOwner;