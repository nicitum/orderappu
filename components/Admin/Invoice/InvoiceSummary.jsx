...
const InvoiceSummary = () => {
    ...
    const loadInvoices = useCallback(async (refresh = false) => {
        ...
        try {
            // Ensure we have the billedBy value
            let billedByUsername = billedBy;
            if (!billedByUsername) {
                billedByUsername = await getBilledByFromToken();
            }
            
            // Check if we have a valid billedByUsername
            if (!billedByUsername) {
                throw new Error('Unable to determine user identity for invoice fetching');
            }
            ...
            const result = await fetchInvoicesByBilledBy(billedByUsername, fromTimestamp, toTimestamp);
    ...
    }, [billedBy, fromDate, toDate, getBilledByFromToken]);
                                ...
                                {item.order_products.map((product, index) => (
                                    <View key={index} style={styles.productItem}>
                                        <View style={styles.productHeader}>
                                            <Text style={[styles.productName, { fontSize: getScaledSize(13) }]}>{product.name}</Text>
                                            <Text style={[styles.productQuantity, { fontSize: getScaledSize(13) }]}>Qty: {product.quantity}</Text>
                                        </View>
                                        <View style={styles.productDetails}>
                                            <Text style={[styles.productPrice, { fontSize: getScaledSize(12) }]}>Price: {formatCurrency(product.price)}</Text>
                                            <Text style={[styles.productCategory, { fontSize: getScaledSize(12) }]}>Category: {product.category}</Text>
                                        </View>
                                        {product.product_code && (
                                            <Text style={[styles.productCode, { fontSize: getScaledSize(12) }]}>Code: {product.product_code}</Text>
                                        )}
                                    </View>
                                ))}
                            </View>
